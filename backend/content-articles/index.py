...
def handler(event: dict, context) -> dict:
    ...
    content_type = params.get("type", "blog")  # blog | training
    ...
    if method == "GET":
        ...
        if article_id:
            cur.execute(f"""
                SELECT id, content_type, title, preview, body, category, status, tags, photos, videos, created_at, updated_at
    ...
    if method == "POST":
        photos = "{" + ",".join(escape(p) for p in (body.get("photos") or [])) + "}"
        ...
        cur.execute(f"""
            INSERT INTO {S}.content_articles (content_type, title, preview, body, category, status, tags, photos, videos)
            VALUES ({escape(body.get("content_type","blog"))}, {escape(body.get("title",""))}, {escape(body.get("preview",""))},
                    ...
                    {escape(body.get("tags",""))}, {photos_val}, {videos_val})
            RETURNING id
        """)
