...
АВТОРИЗАЦИЯ — два способа (оба должны работать):
  1. Header X-Admin-Token: <значение ADMIN_SECRET из секретов>
  2. Header X-User-Id: <id пользователя с is_superadmin=true в БД>

SMTP батчи: send_campaign_bulk — batch_size=10, пауза 1 сек между батчами.
  НЕ МЕНЯТЬ — Gmail блокирует при слишком быстрой массовой отправке.

AUDIENCE_FILTERS — словарь SQL-условий для сегментации рассылки.
  Ключи: all, broker, agency, basic, new, inactive — добавлять только сюда.
"""
import os
import json
import logging
