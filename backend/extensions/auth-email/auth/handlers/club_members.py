...
def handle(event: dict, origin: str = '*') -> dict:
    ...
    try:
        ...
        # Проверяем доступ: broker/agency, суперадмин, активная подписка или сотрудник АН
        if req_status not in ('broker', 'agency') and not is_superadmin:
            sub_row = query_one(
                f"SELECT 1 FROM {S}users WHERE id = {escape(requester_id)} AND subscription_end_at > NOW() LIMIT 1"
            )
            member_row = query_one(
                f"SELECT 1 FROM {S}org_memberships WHERE user_id = {escape(requester_id)} AND status = 'active' LIMIT 1"
            )
            if not sub_row and not member_row:
        ...
        print(f"[club_members] ERROR: {e}\n{traceback.format_exc()}")
        return error(500, f'Внутренняя ошибка: {str(e)}', origin)