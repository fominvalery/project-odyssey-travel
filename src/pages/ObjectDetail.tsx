...
export default function ObjectDetail() {
  ...
  const seoTitle = [obj.title, obj.city || obj.address, obj.price ? `${obj.price} ₽` : null]
    .filter(Boolean)
    .join(" — ")
  const seoDescriptionRaw = [
    obj.type,
    obj.area ? `${obj.area} м²` : null,
    obj.address || obj.city,
    obj.description,
  ].filter(Boolean).join(". ")
  const seoDescription = seoDescriptionRaw.length > 300
    ? seoDescriptionRaw.slice(0, 297) + "..."
    : seoDescriptionRaw
  const seoImage = obj.photos?.[0] || ""
