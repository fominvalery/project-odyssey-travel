// Экран «Папки» — список папок + создание/редактирование/удаление

import { useState } from "react"
import Icon from "@/components/ui/icon"
import { Folder, EMOJIS, saveFolders } from "./messagesDialogMeta"
...
export default function MessagesFolderScreen({ folders, onBack, onSelectFolder, onFoldersChange }: Props) {
  ...
  function save() {
    ...
    let updated: Folder[]
    if (editFolder) {
      updated = folders.map(f => f.id === editFolder.id ? { ...f, name: n, emoji } : f)
    } else {
      updated = [...folders, { id: crypto.randomUUID(), name: n, emoji }]
    }
              ...
              >
                {confirmDelete === f.id ? (
                  <>
                    ...
                    <button onClick={() => deleteFolder(f.id)} className="text-xs px-2 py-1 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30">Да</button>
                    <button onClick={() => setConfirmDelete(null)} className="text-xs px-2 py-1 bg-[#2a2a2a] text-gray-400 rounded-lg hover:bg-[#333]">Нет</button>
                  </>
                ) : (
                  <>
                    <button onClick={e => { e.stopPropagation(); openEdit(f) }} className="p-1.5 text-gray-600 hover:text-white hover:bg-[#2a2a2a] rounded-lg transition-colors">
                      <Icon name="Pencil" className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={e => { e.stopPropagation(); setConfirmDelete(f.id) }} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
