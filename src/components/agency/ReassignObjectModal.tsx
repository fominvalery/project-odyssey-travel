...
interface Props {
  ...
  onReassign: (objId: string, toUserId: string) => Promise<void>
  onClose: () => void
}

export default function ReassignObjectModal({ obj, employees, currentUserId, onReassign, onClose }: Props) {
  const [selectedUserId, setSelectedUserId] = useState("")
  const [loading, setLoading] = useState(false)
  const [search, setSearch] = useState("")

  const available = employees.filter(e =>
    e.user_id !== obj.user_id &&
  ...
  async function handleReassign() {
    ...
    setLoading(true)
    try {
      await onReassign(String(obj.id), selectedUserId)
      onClose()
    } finally {
      setLoading(false)
    }
  ...
  const currentOwner = employees.find(e => e.user_id === obj.user_id)
      ...
      <div className="bg-[#111] border border-[#2a2a2a] rounded-2xl w-full max-w-md shadow-2xl flex flex-col" style={{ maxHeight: "85vh" }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1f1f1f]">
          <div>
            <p className="font-semibold text-white flex items-center gap-2">
              <Icon name="ArrowRightLeft" size={16} className="text-violet-400" />
              Переназначить объект
            </p>
            <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[280px]">{obj.title}</p>
          </div>
        ...
        {currentOwner && (
          <div className="px-5 py-3 border-b border-[#1f1f1f] bg-[#0d0d0d]">
            <p className="text-[11px] text-gray-500 mb-2 uppercase tracking-widest">Текущий владелец</p>
        ...
        <div className="px-5 py-3 border-b border-[#1f1f1f]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a] text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-violet-500/40"
            />
          </div>
        </div>
            ...
            <div className="divide-y divide-[#1a1a1a]">
              {available.map(e => (
                ...
                >
                  ...
                  {selectedUserId === e.user_id && (
                    <Icon name="CheckCircle2" size={16} className="text-violet-400 shrink-0" />
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="px-5 py-4 border-t border-[#1f1f1f] flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-sm text-white bg-violet-600 hover:bg-violet-700 transition-colors disabled:opacity-40 flex items-center justify-center gap-2"
          >
            {loading ? (
